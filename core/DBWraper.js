...
class DBWraper {
    ...
    setStat({ ...params }) {
        ...
        this.field = (typeof params.field === 'undefined') ? [] : params.field;
        this.where = (typeof params.where === 'undefined') ? {} : params.where;
        //this.res = (typeof params.res === 'undefined') ? this.res : params.res;
        //this.req = (typeof params.req === 'undefined') ? this.req : params.req;
    }
    getCmd() {
        return (this.myQuery)
    }
    ...
    setCmd(setQuery) {
        this.myQuery.cmd = setQuery.cmd;
        this.myQuery.arr = setQuery.arr;
    ...
    rowQB(params) {
        ...
        return (
            pool.query(myQuery.cmd, myQuery.arr)
                .then(r => ({ command: r.command, rows: r.rows, rowCount: r.rowCount, cMsg: function () { return `Succesfull ${this.command} # ${this.rowCount} records` } }))
    ...
    insertQB() {
        ...
        Object.entries(field).forEach(
            ([key, val], i) => {
                ...
                if (typeof val !== 'undefined') {
                    kolomlen++;
                    kolom = kolom + key + ((i == Object.entries(field).length) ? '' : ',');
                    isi = isi + `$${kolomlen} ` + ((i == Object.entries(field).length) ? '' : ',');
                    kolomarr.push(val);
                };
